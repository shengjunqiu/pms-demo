...
export function proposeChange(state:BusinessState,id:string,input:ChangeInput){
 ...
 const sum=(id:string)=>sumMoney(items.filter(i=>i.subjectId===id||i.subjectId.startsWith(id+'-')).map(i=>i.amount));
 const budget:BudgetVersion={...structuredClone(currentBudget),id:`PROPOSED-${id}`,version:'变更提议',status:'审批中',details:undefined,items,totalAmount:sumMoney(items.map(i=>i.amount)),laborCost:sum('SUB-01'),outsourceCost:sum('SUB-02'),procurementCost:sum('SUB-03'),expenseCost:sum('SUB-04'),reserveCost:sum('SUB-05')};
...
export type ProjectChangeAction={type:'save-project-change';projectId:string;id?:string;input:ChangeInput}|{type:'submit-project-change';id:string}|{type:'assess-project-change';id:string;area:'技术'|'财务'|'市场';opinion:string}|{type:'classify-project-change';id:string;opinion:string}|{type:'review-project-change';id:string;approve:boolean;opinion:string};
