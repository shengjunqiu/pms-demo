...
export function applyProjectChangeAction(state:BusinessState,action:ProjectChangeAction,actor:Actor){
 ...
 if(action.type==='submit-project-change'){
  ...
  if(actor.role!=='pmo'||r.status!=='待分级')throw new Error('专业评估齐全后由PMO分级');if(!action.opinion.trim())throw new Error('分级依据必填');r.requiredRole=r.ruleReasons.length?'executive':'pmo';r.status=r.requiredRole==='executive'?'PMC审议中':'PMO审批中';r.classifiedBy=actor.name;r.classificationOpinion=action.opinion;summary.status=r.requiredRole==='executive'?'PMC审议中':'PMO审批中';
  ...
  if(actor.role!=='finance')throw new Error('仅财务可办理会签');if(!['PMO审批中','PMC审议中'].includes(r.status))throw new Error('变更须在审批阶段办理会签');if(!action.opinion.trim())throw new Error('会签意见必填');
  ...
  if(!action.opinion.trim())throw new Error('审批意见必填');if(['草稿','已批准','已否决'].includes(r.status))throw new Error('变更尚未提交或已处理');if(action.approve&&(!['PMO审批中','PMC审议中'].includes(r.status)||actor.role!==r.requiredRole))throw new Error('须完成评估与分级，并由指定层级审批');if(!action.approve&&!['pmo','executive'].includes(actor.role))throw new Error('仅PMO或集团领导可退回变更');
  ...
  r.status=action.approve?'已批准':'已否决';r.reviewedBy=actor.name;r.opinion=action.opinion;r.reviewedAt=AS_OF_DATE;summary.status=action.approve?'已批准':'已否决';const decision=state.decisions.find(d=>d.id===`DEC-${r.id}`);if(decision)decision.status=action.approve?'已通过':'已否决';
 }
 return r.id;
}
