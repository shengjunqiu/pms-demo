...
export function initAcceptanceFixture(state: BusinessState) {
  ...
  sources.forEach((s,i)=>{const record=i===0&&sourceRound?sourceRound:{id:`ACC-P006-SUP-${i+1}`,projectId:p.id,type:'供应商验收' as const,round:1,status:'已通过' as const,amount:0,acceptanceDate:date};if(record!==sourceRound)state.acceptances.push(record);record.acceptanceDate=date;const d=acceptanceDetail(state,record);d.scope=s.scope;d.supplierSourceId=s.id;d.plannedDate=date;d.participants='采购主管、项目经理、技术验收组';d.history=[{date,actor:'李主任',action:'历史供应商履约验收通过',note:s.code}];state.acceptanceDetails[record.id]=d;});
