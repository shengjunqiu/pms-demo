...
export function settlementChecks(state: BusinessState, projectId:string, excludeRequestId?:string) {
  ...
  return [
    ...
    {name:'重大事项与变更',passed:!state.issues.some((i)=>i.projectId===projectId&&i.severity==='重大'&&i.status!=='已关闭')&&!state.changes.some((c)=>c.projectId===projectId&&['PMO审批中','PMC审议中'].includes(c.status))&&!state.approvals.some((a)=>a.projectId===projectId&&a.status==='待审批')&&!state.planRequests.some((r)=>r.projectId===projectId&&r.status==='待审批'),detail:'重大问题须最终关闭，变更、预算及计划审批须完成',path:'dynamic-accounting'},
  ];
}
