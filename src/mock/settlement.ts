...
export function settlementSnapshot(state:BusinessState, projectId:string):SettlementSnapshot {
  const p=state.projects.find((p)=>p.id===projectId)!;const c=selectFourCalculations(p,state);const receiptSummary=selectReceipts([p],state);const acceptance=state.acceptances.filter((a)=>a.projectId===p.id&&a.type==='客户终验').sort((a,b)=>b.round-a.round)[0];
  const contracts=receiptSummary.contracts;const receiptPlans=receiptSummary.plans;const saved=state.settlementForecastSnapshots[p.id];const last=mockCostSnapshots.filter((s)=>s.projectId===p.id).at(-1);
  return structuredClone({capturedAt:AS_OF_DATE,acceptanceId:acceptance?.id??'',confirmedAt:acceptance?state.acceptanceDetails[acceptance.id]?.confirmedAt??'':'',estimateId:c.estimate?.id??'',estimateVersion:c.estimate?.version??'',budgetId:c.budget?.id??'',budgetVersion:c.budget?.version??'',baselineId:state.baselines.find((b)=>b.projectId===p.id&&b.status==='已生效')?.id??'',income:receiptSummary.signed,cost:c.actual,committedCost:p.committedCost,forecastRemainingCost:p.forecastRemainingCost,receipts:receiptSummary.paid,receivable:receiptSummary.outstanding,overdue:receiptSummary.overdue,lastRolling:saved?.total??last?.rolling??c.rolling,lastRollingDate:saved?.date??last?.date??AS_OF_DATE,subjects:c.subjects.map((s)=>({subjectId:s.subjectId,subjectName:s.subjectName,estimate:s.estimate,budget:s.budget,actual:s.actual,rolling:saved?.subjects[s.subjectId]??s.rolling})),contracts:contracts.map((c)=>({id:c.id,code:c.code,amount:c.amount,status:c.status})),receiptPlans:receiptPlans.map((r)=>({id:r.id,contractId:r.contractId,amount:r.amount,paidAmount:r.paidAmount,dueDate:r.dueDate})),costs:c.costs.map((c)=>({id:c.id,sourceId:c.sourceId,type:c.type,subjectId:c.subjectId,subjectName:c.subjectName,amount:c.amount,description:c.description})),changes:state.changes.filter((c)=>c.projectId===p.id).map((c)=>({id:c.id,title:c.title,status:c.status,costImpact:c.costImpact}))});
}
...
export function applySettlementAction(state:BusinessState,action:SettlementAction,actor:Actor){
  ...
  if(action.type==='resolve-settlement-source'){
    ...
    const next:SettlementRequest={id,projectId:p.id,version,previousId:reuse?old.previousId:old?.id,status:action.submit?'财务核算':'草稿',note:action.note,files:[...action.files],submittedBy:actor.name,createdAt:AS_OF_DATE,snapshot,history:[...(reuse?old.history:[]),{actor:actor.name,date:AS_OF_DATE,action:action.submit?'提交结算':'保存草稿',opinion:action.note}]};
    ...
    if(action.operation==='amount-return'){if(!finance)throw new Error('只有财务可明确退回金额调整并解除临时冻结');request.status='金额退回';if(state.constructionFreezes[p.id]?.requestId===request.id)delete state.constructionFreezes[p.id];}
      ...
      if(!finance||request.status!=='财务核算')throw new Error('当前步骤由财务核算确认');if(!Number.isFinite(action.invoicedAmount)||action.invoicedAmount!<0||action.invoicedAmount!>request.snapshot.income||!Number.isFinite(action.taxAmount)||action.taxAmount!<0||action.taxAmount!>request.snapshot.income)throw new Error('财务确认开票及税费须为有效金额且不超过收入');request.finance={actor:actor.name,date:AS_OF_DATE,invoicedAmount:action.invoicedAmount!,taxAmount:action.taxAmount!,opinion:action.opinion};request.financeConfirmedAt=AS_OF_DATE;request.status='PMO审核';
    }else if(action.operation==='pmo-audit'){if(!pmo||request.status!=='PMO审核')throw new Error('当前步骤由PMO进行材料与过程审核');request.pmoAuditedAt=AS_OF_DATE;request.status='结算评审';}
    else if(action.operation==='review-pass'){if(!pmo||request.status!=='结算评审')throw new Error('当前步骤由PMO组织结算评审');request.reviewPassedAt=AS_OF_DATE;request.status='待最终锁定';}
    else{
      ...
      const now=settlementSnapshot(state,p.id);if(JSON.stringify(now.costs)!==JSON.stringify(request.snapshot.costs)||now.income!==request.snapshot.income||now.budgetId!==request.snapshot.budgetId||now.estimateId!==request.snapshot.estimateId||now.committedCost!==request.snapshot.committedCost||now.forecastRemainingCost!==request.snapshot.forecastRemainingCost)throw new Error('结算引用金额或版本变化，须财务退回重新申报');
      const margin=money(request.snapshot.income-request.snapshot.cost);state.settlements.push({id:`SET-${request.id}`,projectId:p.id,finalIncome:request.snapshot.income,finalCost:request.snapshot.cost,finalGrossMargin:margin,finalGrossMarginRate:percentage(margin,request.snapshot.income)??0,status:'已锁定已生效',isCostLocked:true,settledDate:AS_OF_DATE,createdAt:AS_OF_DATE,submittedBy:actor.name});state.lockedProjects.push(p.id);delete state.constructionFreezes[p.id];request.status='已锁定';request.lockedAt=AS_OF_DATE;p.status='已结算';if(p.isMaintenance){p.phase='运维';p.subPhase='质保运维';}else{p.phase='收尾';p.subPhase='运维交接';}
    }
