...
export type ReportAction = { type: 'save-daily'; projectId: string; submit: boolean; progress: number; hasProgress: boolean; completed: string; reason: string; nextPlan: string; coordination: string; linkedIds: string[] }
...
export function applyReportAction(state: BusinessState, action: ReportAction, actor: Actor) {
  ...
  if (action.type === 'save-daily') {
    ...
    const report: DailyReport = { id: previous?.id ?? `DR-NEW-${state.dailyReports.length + 1}`, projectId: p.id, date: AS_OF_DATE, reporter: actor.name, completedTasks: action.hasProgress ? action.completed.trim() : `无进展：${action.reason.trim()}`, plannedTasks: action.nextPlan.trim(), spentHours: 0, status: action.submit ? '已提交' : '草稿', reportedProgress: action.progress, hasProgress: action.hasProgress, noProgressReason: action.reason, coordination: action.coordination.trim(), linkedIds: [...new Set(action.linkedIds)], snapshot: reportSnapshot(state, p.id) };
