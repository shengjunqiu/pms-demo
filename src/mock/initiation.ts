...
export function canViewInitiation(state:BusinessState,app:InitiationApplication,actor:Actor){const o=state.opportunities.find(o=>o.id===app.input.opportunityId);return actor.role!=='market'||!!o&&canManageOpportunity(state,o,actor);}
...
export function applyInitiationAction(state:BusinessState,action:InitiationAction,actor:Actor){
 ...
 if(action.type==='submit-initiation'){
  ...
  if(action.type==='assess-initiation-risk'){
   ...
   const node=INITIATION_SIGNATURES.find(n=>n.node===action.node);if(!node||node.role!==actor.role||round.status!=='会签中')throw new Error('仅指定角色可签署当前会签节点');if(!action.opinion.trim()||round.signatures.some(s=>s.node===node.node))throw new Error('意见必填且同一节点不能重复签署');round.signatures.push({node:node.node,role:actor.role,by:actor.name,conclusion:action.conclusion,opinion:action.opinion,date:AS_OF_DATE});if(round.signatures.length===INITIATION_SIGNATURES.length){round.status='待决策';app.status=round.status;}
   ...
   if(action.result==='通过'){
    ...
    if(progress){reviewConfiguredApproval(progress,actor,true,action.opinion);if(progress.status!=='通过')return app.id;}
    createInitiatedProject(state,app,actor);
   }else {if(action.result==='否决'&&progress&&round.status==='待决策'&&currentNode?.roles.includes(actor.role)){reviewConfiguredApproval(progress,actor,false,action.opinion);if(progress.status!=='驳回')return app.id;}else if(actor.role!==decisionRole)throw new Error('本路径整改、否决或暂缓须由指定决策角色办理');}
