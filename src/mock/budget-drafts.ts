...
export const BUDGET_SUBJECTS=[{id:'SUB-01',name:'直接人力成本'},{id:'SUB-02',name:'外包开发成本'},{id:'SUB-03',name:'采购与设备'},...['差旅费','住宿费','业务招待费','其他期间费用'].map((name,i)=>({id:`SUB-04-${i+1}`,name})),{id:'SUB-05',name:'不可预见准备金'}];
export const lineAmount=(line:BudgetLine)=>line.kind==='labor'?money(line.plannedDays*BUDGET_RULE.workingHoursPerDay*hourlyRate(line.userId??'')/10000):money(line.amount);
...
export function toBudgetVersion(draft:BudgetDraft,actor:Actor):BudgetVersion {
 const items=BUDGET_SUBJECTS.map(s=>({subjectId:s.id,subjectName:s.name,amount:sumMoney(draft.lines.filter(l=>l.subjectId===s.id).map(lineAmount))}));const amount=(id:string)=>sumMoney(items.filter(i=>i.subjectId===id||i.subjectId.startsWith(id+'-')).map(i=>i.amount));
...
export function saveBudgetDraft(state:BusinessState,draft:BudgetDraft,expectedRevision:number,actor:Actor){
 const p=state.projects.find(p=>p.id===draft.projectId);if(!p)throw new Error('项目不存在');assertConstructionWritable(state,p.id);if(actor.role!=='project-manager'||actor.id!==p.pmId)throw new Error('仅项目主PM可编制预算');if(state.approvals.some(a=>a.projectId===p.id&&(a.status==='待审批'||a.kind==='budget'&&a.status==='通过'&&!a.baselineConfirmedAt)))throw new Error('审批或基线确认中不可修改预算草稿');if((state.budgetDrafts[p.id]?.revision??0)!==expectedRevision)throw new Error('预算草稿版本已变化');validateBudgetDraft(state,draft);state.budgetDrafts[p.id]={...structuredClone(draft),revision:expectedRevision+1,updatedAt:AS_OF_DATE};return p.id;
}
