...
export function assertArchiveActionWritable(state:BusinessState,action:{type:string;projectId?:string;opportunityId?:string;id?:string;approvalId?:string;cost?:{projectId:string};report?:{projectId:string};maintenance?:boolean}){
 ...
 if(!projectId&&id){
   ...
   const record=records.find((r):r is {id:string;projectId:string}=>typeof r==='object'&&r!==null&&'id' in r&&r.id===id&&'projectId' in r&&typeof r.projectId==='string');const opportunity=state.opportunities.find((o)=>o.id===id);const estimate=state.estimates.find((e)=>e.id===id);
   if(!project&&!record&&!opportunity&&!estimate)throw new Error('原业务对象不存在，不能跳过归档只读校验');
   projectId=project?.id??record?.projectId??state.projects.find((p)=>p.opportunityId===(opportunity?.id??estimate?.opportunityId))?.id;
 }
