...
export function TicketDetailPage() {
  const { id } = useParams();
  return <TicketDetailContent key={`ticket-detail-${id}`} />;
}
...
function TicketDetailContent() {
  ...
  const closed = ticket.status === '已关闭'; const base = '/tickets';
  ...
  const orderedOperations: Operation[] = [primary, ...(kind === 'risk' ? ['convert' as const] : awaiting ? ['return' as const] : []), 'feedback', 'transfer', ...(kind !== 'risk' ? awaiting ? ['resolve' as const] : ['confirm' as const, 'return' as const] : ['escalate' as const])];
  ...
  return <>
    <PageHeader title={`${ticketLabels[kind]}详情`} description={`${ticket.id} · ${p.id} · ${p.name}`} breadcrumbs={[{ title: '首页', href: '/' }, { title: '事项管理', href: `${base}?${params}` }, { title: '原事项' }]} extra={<Button onClick={() => navigate(`${base}?${params}`)}>返回台账</Button>} />
    ...
    {(sourceRisk || converted) && <PageSection title="风险与问题关联" description="保留原风险，实际问题独立处理并由主PM关闭。"><Space wrap>{sourceRisk && <Button onClick={() => navigate(`/tickets/${sourceRisk}?${params}`)}>查看来源风险 {sourceRisk}</Button>}{converted && <Button onClick={() => navigate(`/tickets/${converted.id}?${params}`)}>查看已转问题 {converted.id}</Button>}</Space></PageSection>}
    <Row gutter={16}>
      <Col xs={24} lg={15}>
        <PageSection title="原始事项与处理依据">
          ...
          {kind === 'requirement' && <><h3 style={{ fontSize: 14, margin: '20px 0 8px' }}>产品与基线关联</h3><Descriptions size="small" column={1} items={[
            ...
            { key: 'change', label: '关联变更', children: meta.changeRequestId ? <Button type="link" onClick={() => navigate(`/projects/${p.id}/plan-requests/${meta.changeRequestId}?ticketQuery=${encodeURIComponent(params.toString())}`)}>{meta.changeRequestId}</Button> : '无' },
          ]} />{meta.baselineImpact && <><Alert style={{ margin: '12px 0' }} showIcon type="info" message="关联计划变更通过后，方可提交需求解决结果。" /><Button disabled={!pm || !!meta.changeRequestId || !canDo('request-plan', p.id)} onClick={() => navigate(`/projects/${p.id}/plan-requests/new?ticketQuery=${encodeURIComponent(params.toString())}&kind=schedule&sourceRequirementId=${ticket.id}&reason=${encodeURIComponent(`需求${ticket.id}影响进度：${ticket.title}`)}`)}>转计划变更申请</Button></>}</>}
    ...
    <PageToolbar><Button onClick={() => navigate(`/projects/${p.id}?tab=tickets`)}>查看项目原摘要</Button><span style={{ color: '#64748b', fontSize: 12 }}>项目、台账与原事项共享同一业务状态。</span></PageToolbar>
