...
export function TicketDetailPage({ family }: { family: 'quality' | 'risk' }) {
  ...
  return <><PageHeader title={`${family === 'quality' ? 'HS-06' : 'HS-08'} ${ticketLabels[kind]}详情`} description={`${ticket.id} · ${p.name}`} breadcrumbs={[{ title: '首页', href: '/' }, { title: '原业务详情' }]} extra={<Button onClick={() => navigate(`${base}?${params}`)}>返回台账</Button>} />
    ...
    <Modal title={operation ? operations[operation] : ''} open={!!operation} okButtonProps={{disabled:!canOperateCurrent}} onCancel={() => setOperation(undefined)} onOk={() => {
      try { if(!canOperate(operation))throw new Error('当前策略或事项状态不允许此操作');if (!note.trim()) throw new Error('处理说明或评价必填'); const actor = { id: currentUser.id, name: currentUser.name, role: currentRole }; if (operation === 'convert') dispatch({ type: 'risk-to-issue', id: ticket.id, note }, actor); else dispatch({ type: 'update-ticket', kind, id: ticket.id, operation: operation!, note, ownerId }, actor); setOperation(undefined); message.success('原事项及关联摘要已更新'); } catch (error) { message.error((error as Error).message); }
    }}><p>{ticket.title}</p>{operation === 'transfer' && <Select disabled={!canOperateCurrent} aria-label="转办责任人" style={{ width: '100%', marginBottom: 12 }} placeholder="选择另一位责任人" showSearch optionFilterProp="label" value={ownerId} options={mockUsers.map((u) => ({ value: u.id, label: u.name }))} onChange={setOwnerId} />}<Input.TextArea disabled={!canOperateCurrent} aria-label="处理说明" rows={4} placeholder="处理结果、转办原因、措施或验证评价（必填）" value={note} onChange={(e) => setNote(e.target.value)} /></Modal>
  </>;
}
