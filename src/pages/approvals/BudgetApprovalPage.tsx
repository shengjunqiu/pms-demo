...
export function BudgetApprovalPage({ approvalId, embedded = false }: { approvalId?: string; embedded?: boolean } = {}) {
 const {canDo}=useActionAccess();
  ...
  return <>{!embedded && <PageHeader title={approval.kind === 'change' ? '重大成本变更原审批' : '预算调整原审批'} description={`${approval.id} · ${project.name} · 审批引用提交时快照`} breadcrumbs={[{ title: '首页', href: '/' }, { title: '预算调整原审批' }]} extra={<Button onClick={() => navigate(-1)}>返回来源</Button>} />}
    ...
    {approval.status === '通过' && approval.kind === 'budget' && <Card style={{marginTop:16}} title="PMO基线确认"><Alert type={approval.baselineConfirmedAt?'success':'info'} message={approval.baselineConfirmedAt ? `基线已于${approval.baselineConfirmedAt}由${approval.baselineConfirmedBy}确认` : '预算审批已通过，原预算仍有效；确认完整快照后正式生效。'} /><p>范围：{approval.baseline.snapshot?.scope}</p><p>WBS {approval.baseline.snapshot?.tasks.length ?? 0} 项 · 里程碑 {approval.baseline.snapshot?.milestones.length ?? 0} 项 · 团队资源 {approval.baseline.snapshot?.resources.length ?? 0} 人</p><Button type="primary" disabled={currentRole!=='pmo'||!!approval.baselineConfirmedAt||!canDo('confirm-budget-baseline',approval.id)} onClick={()=>modal.confirm({title:'确认完整基线正式生效',content:'确认后追加基线版本并冻结计划，历史版本保留。',onOk:()=>{try{if(!canDo('confirm-budget-baseline',approval.id))throw new Error('当前策略不允许确认基线');dispatch({type:'confirm-budget-baseline',approvalId:approval.id},{id:currentUser.id,name:currentUser.name,role:currentRole});message.success('基线已正式生效');}catch(error){message.error((error as Error).message);return Promise.reject(error);}}})}>确认基线生效</Button></Card>}
    <Modal title={decision ? approval.kind === 'change' ? '确认通过并追加新基线' : '确认通过，等待PMO基线确认' : '确认驳回，保留原基线'} okButtonProps={{disabled:!permitted}} open={decision !== undefined} onCancel={() => setDecision(undefined)} onOk={() => {
      try { if(!permitted)throw new Error('当前策略不允许审批');dispatch({ type: 'review', approvalId: approval.id, approve: decision!, opinion }, { id: currentUser.id, name: currentUser.name, role: currentRole }); setDecision(undefined); message.success('审批已记录，原事项状态已更新'); } catch (error) { message.error((error as Error).message); }
