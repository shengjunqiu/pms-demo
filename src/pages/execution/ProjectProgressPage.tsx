...
export function ProjectProgressPage() {
  ...
  return <><PageHeader title="HS-02 项目进度" description={`${p.id} · ${p.name} · 基准日 ${AS_OF_DATE}`} breadcrumbs={[{ title: '首页', href: '/' }, { title: p.name, href: `/projects/${p.id}` }, { title: '项目进度' }]} extra={<Button onClick={() => navigate(-1)}>返回上一级</Button>} />
    <Space wrap style={{ marginBottom: 16 }}><Tag>有效计划基线 {p.currentBaselineVersion}</Tag><Button disabled={!pm||!canDo('request-plan',p.id)} onClick={() => navigate(`/projects/${p.id}/plan-requests/new?kind=schedule&${params}`)}>发起计划变更</Button><Button disabled={!pm||!canDo('request-plan',p.id)} onClick={() => navigate(`/projects/${p.id}/plan-requests/new?kind=stage&${params}`)}>发起阶段切换</Button><Button onClick={() => navigate(`/projects/${p.id}?tab=progress`)}>项目总览</Button></Space>
