...
import { mockUsers, mockRequirements, mockWbsTasks, mockChanges, mockDailyReports, mockWeeklyReports, mockProcurements, mockOutsources, mockDepartments, mockOpportunities, AS_OF_DATE } from '@/mock';
...
export function ProjectOverviewPage() {
  ...
  const requirements = mockRequirements.filter((r) => r.projectId === p.id);
  ...
  const overview = <Row gutter={20}>
    <Col span={14}><Card title="项目经营摘要" size="small"><Row gutter={[12, 12]}>{[
      ...
      <Card title="最近动态" size="small" style={{ marginTop: 16 }}>{mockDailyReports.filter((r) => r.projectId === p.id).slice(0, 1).map((r) => <div key={r.id}><Text strong>{r.date} · {r.reporter}</Text><p>{r.completedTasks}</p><Button size="small" onClick={() => selectTab('reports')}>查看日报周报</Button></div>)}</Card>
    </Col>
  ...
  const tabs = [
    ...
    { key: 'reports', label: '日报周报', children: <><Table rowKey="id" size="small" pagination={{ pageSize: 5 }} dataSource={mockDailyReports.filter((r) => r.projectId === p.id)} columns={[{ title: '日期', dataIndex: 'date' }, { title: '填报人', dataIndex: 'reporter' }, { title: '完成工作', dataIndex: 'completedTasks' }, { title: '下一步', dataIndex: 'plannedTasks' }]} /><Table rowKey="id" size="small" dataSource={mockWeeklyReports.filter((r) => r.projectId === p.id)} columns={[{ title: '周报周期', dataIndex: 'weekSpan' }, { title: '进度摘要', dataIndex: 'progressSummary' }, { title: '下周计划', dataIndex: 'nextWeekPlan' }]} /></> },
    { key: 'risks', label: `问题风险（${riskRows.length}）`, children: <Table rowKey="id" size="small" pagination={{ pageSize: 8 }} dataSource={riskRows} columns={[{ title: '类型', dataIndex: 'kind' }, { title: '标题', dataIndex: 'title', render: (title: string, r) => <Button type="link" onClick={() => open(title, [['编号', r.id], ['类型', r.kind], ['等级', r.level], ['责任人', r.owner], ['状态', r.status], ['日期', r.date], ['来源/策略', r.source]])}>{title}</Button> }, { title: '等级', dataIndex: 'level' }, { title: '责任人', dataIndex: 'owner' }, { title: '状态', dataIndex: 'status', render: marker }]} /> },
    { key: 'requirements', label: `需求BUG（${demandRows.length}）`, children: <><Select aria-label="事项类型" value={params.get('kind') ?? '全部'} style={{ width: 140, marginBottom: 12 }} options={['全部', '需求', 'BUG'].map((value) => ({ value, label: value }))} onChange={(value) => selectTab('requirements', value === '全部' ? undefined : value)} /><Table rowKey="id" size="small" pagination={{ pageSize: 8 }} dataSource={demandRows.filter((row) => !params.get('kind') || row.kind === params.get('kind'))} columns={[{ title: '类型', dataIndex: 'kind' }, { title: '标题', dataIndex: 'title', render: (title: string, r) => <Button type="link" onClick={() => open(title, [['编号', r.id], ['发起人', r.creator], ['责任人', r.owner], ['优先级/严重度', r.level], ['状态', r.status]])}>{title}</Button> }, { title: '责任人', dataIndex: 'owner' }, { title: '状态', dataIndex: 'status', render: marker }] } /></> },
    { key: 'supply', label: '采购外包', children: <Table rowKey="id" size="small" dataSource={[...mockProcurements.filter((r) => r.projectId === p.id).map((r) => ({ ...r, title: r.supplierName })), ...mockOutsources.filter((r) => r.projectId === p.id).map((r) => ({ ...r, title: r.vendorName }))]} columns={[...baseColumns, { title: '单据金额（万元）', dataIndex: 'amount', render: (v: number) => <MoneyText value={v} /> }]} /> },
    { key: 'changes', label: '变更', children: <Table rowKey="id" size="small" dataSource={mockChanges.filter((c) => c.projectId === p.id)} columns={[...baseColumns, { title: '成本影响（万元）', dataIndex: 'costImpact', render: (v: number) => <MoneyText value={v} signed /> }, { title: '工期影响（天）', dataIndex: 'scheduleImpactDays' }]} /> },
    ...
    { key: 'receipts', label: '回款', children: <><Space size={24}><Statistic title="签约金额（万元）" value={receipt.signed} formatter={() => <MoneyText value={receipt.signed} />} /><Statistic title="已收（万元）" value={receipt.paid} formatter={() => <MoneyText value={receipt.paid} />} /><Statistic title="合同剩余应收（万元）" value={receipt.outstanding} formatter={() => <MoneyText value={receipt.outstanding} />} /></Space><Table rowKey="id" size="small" dataSource={receipt.contracts} columns={[{ title: '合同编号', dataIndex: 'code' }, { title: '合同名称', dataIndex: 'name' }, { title: '实收（万元）', dataIndex: 'paidAmount', render: (v: number) => <MoneyText value={v} /> }]} /><Text type="secondary">未签项目的预计收入不计入合同及回款；剩余应收不等同于逾期应收。</Text></> },
