...
export function ProjectOverviewPage() {
  ...
  const tabs = [
    ...
    { key: 'reports', label: '项目报告', children: <><Space style={{ marginBottom: 12 }}><Button onClick={() => navigate(`/projects/${p.id}/reports`)}>进入项目报告</Button></Space><Table rowKey="id" size="small" pagination={{ pageSize: 5 }} dataSource={data.dailyReports.filter((r) => r.projectId === p.id)} columns={[{ title: '日期/周期', render: (_, r) => r.date ?? r.weekSpan }, { title: '类型', render: (_, r) => 'date' in r ? <Tag>日报</Tag> : <Tag color="blue">周报</Tag> }, { title: '填报人', dataIndex: 'reporter' }, { title: '内容摘要', render: (_, r) => r.completedTasks ?? r.progressSummary }]} /></> },
  ...
  const groups = [
    { key: 'overview', label: '项目总览', tabs: ['overview', 'team', 'reports', 'audit'] },
