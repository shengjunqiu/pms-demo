...
export function ProjectOverviewPage() {
  ...
  const tabs = [
    ...
    { key: 'daily', label: '日报', children: <><Space style={{ marginBottom: 12 }}><Button onClick={() => navigate(`/projects/${p.id}/daily-reports`)}>填报或查看日报</Button></Space><Table rowKey="id" size="small" pagination={{ pageSize: 5 }} dataSource={data.dailyReports.filter((r) => r.projectId === p.id)} columns={[{ title: '日期', dataIndex: 'date' }, { title: '填报人', dataIndex: 'reporter' }, { title: '完成工作', dataIndex: 'completedTasks' }, { title: '下一步', dataIndex: 'plannedTasks' }]} /></> },
    { key: 'weekly', label: '周报', children: <><Space style={{ marginBottom: 12 }}><Button onClick={() => navigate(`/projects/${p.id}/weekly-reports`)}>生成或查看周报</Button></Space><Table rowKey="id" size="small" dataSource={data.weeklyReports.filter((r) => r.projectId === p.id)} columns={[{ title: '周报周期', dataIndex: 'weekSpan' }, { title: '进度摘要', dataIndex: 'progressSummary' }, { title: '下周计划', dataIndex: 'nextWeekPlan' }]} /></> },
  ...
  const groups = [
    { key: 'overview', label: '项目总览', tabs: ['overview', 'team', 'daily', 'weekly', 'audit'] },
    { key: 'contract', label: '合同台账', tabs: ['contract-ledger'] },
    { key: 'project-cost', label: '项目成本', tabs: ['project-cost'] },
    { key: 'delivery', label: '计划与交付', tabs: ['progress', 'quality', 'deliverables', 'supply'] },
    { key: 'collaboration', label: '协作事项', tabs: ['risks', 'requirements', 'changes'] },
