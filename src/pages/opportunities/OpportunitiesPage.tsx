...
import { Alert, Button, Col, DatePicker, Form, Input, InputNumber, Row, Select, Space, Table, Tag } from 'antd';
import {
  FileSearchOutlined,
  DollarOutlined,
  ScheduleOutlined,
...
export function OpportunitiesPage() {
 const {canDo}=useActionAccess();
  ...
  const [visible,setVisible]=useState(['customer','owner','sign']);
  ...
  const cols:ColumnsType<Opportunity>=[
    {title:'商机编号 / 名称',key:'name',width:235,fixed:'left',render:(_,o)=><><Link to={`/opportunities/${o.id}`}>{o.name}</Link><div className="font-mono text-xs text-slate-400">{o.code}</div></>},
    {title:'客户',key:'customer',dataIndex:'customerName',width:160}, {title:'负责人',key:'owner',dataIndex:'ownerName',width:90}, {title:'主办部门',key:'department',dataIndex:'departmentName',width:140},
    ...
    {title:'操作',key:'actions',width:112,fixed:'right',render:(_,o)=><OpportunityActions opportunity={o} compact/>},
  ];
  ...
  return <><PageHeader item={PAGE_MANIFEST.find(p=>p.id==='GS-01')} breadcrumbs={[{title:'商机与概算'},{title:'商机台账'}]} description="预计金额与已签合同分开管理；按当前角色可见范围汇总，金额单位：万元。" extra={actor.role==='market'&&<Button type="primary" disabled={!canDo('save-opportunity')} onClick={()=>navigate('/opportunities/new')}>新建商机</Button>}/>
    <Row gutter={16} style={{marginBottom:16}}>{[
      {title:'当前筛选商机',value:String(rows.length),unit:'个',icon:<FileSearchOutlined />,statusText:'当前可见范围',statusType:'info' as const},
      {title:'预计项目总额',value:sumMoney(rows.map(o=>o.estimatedAmount)),unit:'万元',icon:<DollarOutlined />,statusText:'意向金额，尚未签约',statusType:'info' as const},
    ...
    </Row><Space><Button type="primary" htmlType="submit">查询</Button><Button onClick={reset}>重置</Button><Button type="link" onClick={()=>setMore(!more)}>{more?'收起':'更多筛选'}</Button></Space></Form></PageSection>
    <PageSection title={`商机列表 · ${rows.length} 项`} description="默认展示核心商务字段；评估、概算、投入与跟进信息可通过列设置显示。"><PageToolbar><AnalysisTools storageKey={`opportunities-views-${actor.id}`} params={params} onChange={v=>setParams(v)} columns={cols.filter(c=>!['name','amount','status','actions'].includes(String(c.key))).map(c=>({value:String(c.key),label:String(c.title)}))} visible={visible} onColumns={setVisible} exportRows={[["商机编号","商机名称","客户","状态","预计金额（万元）"],...rows.map(o=>[o.code,o.name,o.customerName,o.status,String(o.estimatedAmount)])]}/></PageToolbar><Table size="small" rowKey="id" dataSource={rows} columns={cols.filter(c=>['name','amount','status','actions'].includes(String(c.key))||visible.includes(String(c.key)))} scroll={{x:cols.filter(c=>['name','amount','status','actions'].includes(String(c.key))||visible.includes(String(c.key))).reduce((sum,c)=>sum+Number(c.width??120),0)}} pagination={{pageSize:10,showSizeChanger:true,showTotal:n=>`共 ${n} 条`}}/></PageSection>
  </>;
}
