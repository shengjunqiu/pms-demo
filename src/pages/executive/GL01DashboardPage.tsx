...
export function GL01DashboardPage() {
  ...
  const columnsByTab = {
    ...
    margin: [{ title: '预计收入', dataIndex: 'revenueAmount', render: (v: number) => <MoneyText value={v} /> }, { title: '预测毛利', dataIndex: 'gross', render: (v: number) => <MoneyText value={v} /> }, { title: '预测毛利率', dataIndex: 'grossRate', render: (v: number | null) => formatPercent(v) }],
  ...
  const content = <>
    ...
    ].map((m) => <Col span={6} key={m.label}><Card size="small"><Button type="text" style={{ height: 'auto', width: '100%', textAlign: 'left', padding: 0 }} disabled={!scope.length} onClick={m.action}><Statistic title={m.label} value={m.value ?? 0} valueStyle={{ fontSize: 23 }} formatter={() => m.count ? String(m.value) : m.ratio ? formatPercent(m.value) : <MoneyText value={m.value} />} /></Button></Card></Col>)}</Row>
    <Space wrap size={[20, 8]} style={{ marginBottom: 16 }}><span>预计项目收入 <MoneyText value={kpi.totalRevenue} /></span><span>冻结概算 <MoneyText value={estimate} /></span><span>预算毛利 <MoneyText value={sumMoney([kpi.totalRevenue, -kpi.totalBudgetAmount])} /></span><span>预测毛利率 {formatPercent(kpi.weightedGrossMarginRate)}</span><span>毛利偏差 <MoneyText value={-kpi.totalCostVariance} signed /></span><span>已收 <MoneyText value={receipt.paid} /></span><Button type="link" onClick={() => exception({ exception: 'receipt' })}>逾期 <MoneyText value={receipt.overdue} /></Button></Space>
    ...
    {!scope.length ? <Empty description="当前筛选无项目，请调整条件" /> : <>
      ...
      <Row gutter={16} style={{ marginTop: 16 }}><Col span={14}><Card size="small" title={`重点异常项目（${exceptions.length}）`} extra={<Button type="link" onClick={() => exception()}>全部异常</Button>}><Table rowKey="id" size="small" pagination={false} dataSource={[...exceptions].sort((a, b) => (b.health === 'red' ? 1 : 0) - (a.health === 'red' ? 1 : 0) || b.calc.variance - a.calc.variance).slice(0, 5)} columns={[{ title: '项目', render: (_, p) => <Button type="link" style={{ whiteSpace: 'normal', textAlign: 'left' }} onClick={() => goProject(p.id)}>{p.name}</Button> }, { title: '原因', dataIndex: 'healthReason' }]} /></Card></Col><Col span={10}><Card size="small" title={`待决策（${pending.length}）`} extra={<Button type="link" onClick={() => navigate(`/executive/decisions?${query()}`)}>全部事项</Button>}>
