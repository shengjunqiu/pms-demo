...
// Normalize values only at display precision; keep business amounts unchanged.
const displayDecimal = (value: number, digits = 2) => Number(value.toFixed(digits)).toFixed(digits);
export function BudgetSummary({budget,estimate,income,hideCards=false}:{budget:BudgetVersion;estimate:EstimateVersion;income:number;hideCards?:boolean}){
...
export function BudgetComparisonTable({budget,estimate}:{budget:BudgetVersion;estimate:EstimateVersion}){
 ...
 return <Table rowKey="id" size="small" dataSource={rows} pagination={false} columns={[{title:'统一叶子科目',dataIndex:'name'},{title:'概算（万元）',dataIndex:'estimate',align:'right',render:v=>displayDecimal(v)},{title:'预算（万元）',dataIndex:'budget',align:'right',render:v=>displayDecimal(v)},{title:'差异（万元）',dataIndex:'diff',align:'right',render:v=><span style={{color:v>0?'#cf1322':undefined}}>{Number(displayDecimal(v))>0?'+':''}{displayDecimal(v)}</span>},{title:'差异率',align:'right',render:(_,r)=>{const v=percentage(r.diff,r.estimate);return v===null?'—（零概算）':`${displayDecimal(v,1)}%`;}}]}/>;
}
