...
export function ChangeFormPage() {
    ...
    return <ChangeFormContent key={`${params.get("changeId") ?? `new:${params.get("projectId") ?? "P-001"}`}:${currentUser.id}`}/>;
}
function ChangeFormContent() {
    ...
    const [assessmentArea, setAssessmentArea] = useState<"技术" | "财务" | "市场">(currentRole === "finance" ? "财务" : currentRole === "market" ? "市场" : "技术");
    ...
    const input = local?.projectId === p.id
        ? local.input
        : (request?.input ?? defaultChangeInput(data, p.id));
    const lockReason = constructionLockReason(data, p.id) ?? (["已终止", "已关闭"].includes(p.status) ? "项目已终止或关闭" : undefined);
    const editableState =
        !lockReason && currentRole === "project-manager" &&
        currentUser.id === p.pmId &&
        (!request || ["草稿", "驳回"].includes(request.status));
    const canEdit = editableState && canDo("save-project-change", request?.id ?? p.id);
    const canSubmit = editableState &&
        request?.status === "草稿" &&
        !local &&
        canDo("submit-project-change", request.id);
    const canSelectAssessment = !lockReason && !!request &&
        ["影响评估中", "待分级"].includes(request.status) &&
        ["solution-tech", "finance", "market"].includes(currentRole) &&
        canDo("assess-project-change", request.id);
    const canAssess = canSelectAssessment &&
        currentRole ===
            { 技术: "solution-tech", 财务: "finance", 市场: "market" }[assessmentArea];
    ...
    const update = (patch: Partial<ChangeInput>) => {
        if (!editableState || !canDo("save-project-change", request?.id ?? p.id))
            return;
        setLocal({ projectId: p.id, input: { ...input, ...patch } });
    };
    const showMargin = !!canViewSensitiveField(data, { role: currentRole }, "margin");
    let proposal: ReturnType<typeof proposeChange> | undefined, error = "";
    try {
        proposal =
            request && !canEdit
                ? {
                    ...
                    reasons: request.ruleReasons,
                }
                : proposeChange(data, p.id, input);
    }
    catch (e) {
    ...
    const save = () => {
        if (!editableState || !canDo("save-project-change", request?.id ?? p.id))
            return false;
        if (run({
            type: "save-project-change",
            projectId: p.id,
            id: request?.id,
            input,
        })) {
            const latest = useBusinessStore.getState().data.changeRequests.at(-1)!;
            const next = new URLSearchParams(window.location.search);
            next.set("changeId", request?.id ?? latest.id);
            setParams(next);
            setLocal(undefined);
            return true;
        }
    ...
    const base = proposal?.original, proposed = proposal?.budget;
    const adjustments = base
        ? (base.snapshot?.budget.items ?? data.budgets.find((b) => b.projectId === p.id && b.version === base.version)?.items ??
            data.budgets.find((b) => b.projectId === p.id && b.status === "已生效")
                ?.items ??
            [])
        : [];
    const returnParams = new URLSearchParams(params);
    returnParams.delete("changeId");
    const listUrl = `/project-changes?${returnParams}`;
    return (<>
      <PageHeader title={request?.input.title ?? "发起项目变更"} description={`${request?.id ?? "新变更"} · ${p.id} · ${p.name}`} breadcrumbs={[
            { title: "首页", href: "/" },
            { title: "项目变更", href: listUrl },
            { title: "变更申请与评审" },
        ]} extra={<Space>
            <Button onClick={() => navigate(listUrl)}>
              返回台账
            </Button>
            {editableState && (<Button disabled={!canEdit} onClick={save}>
                保存草稿
              </Button>)}
            {request?.status === "草稿" && (<Button type="primary" disabled={!canSubmit} onClick={() => modal.confirm({
                    title: "提交变更影响评估",
                    content: "提交后锁定本轮提议，技术、财务和市场分别评估，再由PMO分级。",
                    okButtonProps: { disabled: !canSubmit },
                    onOk: () => {
                        if (!canDo("submit-project-change", request.id))
                            return Promise.reject(new Error("当前策略禁止提交变更"));
                        return run({
                            type: "submit-project-change",
                            id: request.id,
                        });
                    },
                })}>
      ...
      <Space className="pms-record-summary" wrap style={{ marginBottom: 16 }}>
        <Tag data-testid="change-status" color="blue">{request?.status ?? "草稿"}</Tag>
        <Tag>
          {request?.requiredRole === "executive" ? "集团领导 / PMC" : "PMO路径"}
        </Tag>
        <Select aria-label="变更所属项目" style={{ width: 350 }} disabled={!!request} value={p.id} showSearch optionFilterProp="label" options={visibleProjects(currentRole, data.projects, data)
            .filter((x) => data.baselines.some((b) => b.projectId === x.id && b.status === "已生效"))
            .map((x) => ({ value: x.id, label: `${x.id} ${x.name}` }))} onChange={(id) => {
            const next = new URLSearchParams(window.location.search);
        ...
        <Button onClick={() => navigate(`/projects/${p.id}/baseline`)}>
          查看原基线
        </Button>
      </Space>
    ...
    {!request?.assessments.length && <p>尚无专业评估意见</p>}{request?.classificationOpinion && <p>分级依据：{request.classificationOpinion}</p>}</Card>
          <Card title="版本与留痕" size="small" style={{ marginTop: 16 }}>
            <Descriptions column={1} items={[
            {
                key: "revision",
                label: "申请修订",
                children: request?.revision ?? "未保存",
            },
            {
                key: "source",
                label: "来源基线",
                children: request?.original.id ?? base?.id,
            },
            {
                key: "actor",
                label: "提交人",
                children: request?.submittedBy ?? currentUser.name,
            },
            {
                key: "classified",
                label: "分级人",
                children: request?.classifiedBy ?? "待分级",
            },
        ]}/>
            {request?.history?.map((h) => (<details key={h.revision} style={{ marginTop: 12 }}>
                <summary>修订{h.revision} · {h.status}</summary>
                <p>{h.opinion}</p>
                <p>{h.input.title} · {h.input.reason}</p>
                <p>客户依据：{h.input.customerBasis}</p><p>合同依据：{h.input.contractBasis}</p>
                <p>范围：{h.original.scopeDesc} → {h.proposed.scope}</p>
                <p>计划验收：{h.original.plannedEndDate} → {h.proposed.plannedEndDate}</p>
                <p>类型：{h.input.type} · 紧急程度：{h.input.urgency}</p>
                <p>预算万元：{h.original.budgetAmount.toFixed(2)} → {h.proposed.budget.totalAmount.toFixed(2)}</p>
                <p>目标收入万元：{h.input.proposedIncome.toFixed(2)} · 计划顺延：{h.input.shiftDays}天</p>
                <p>新增工作包：{h.input.newWorkPackage || "无"} · 责任人：{h.input.newTaskOwnerId}</p>
                <p>重大风险：{h.input.majorRisk ? "是" : "否"}</p>
                <Table size="small" rowKey="userId" pagination={false} dataSource={h.proposed.resources} columns={[{title:"资源",dataIndex:"name"},{title:"计划工时",dataIndex:"plannedHours"}]} />
                <p>采购：{h.input.procurementImpact}</p><p>外包：{h.input.outsourceImpact}</p>
                <p>风险：{h.input.risk}</p><p>附件：{h.input.attachments.join("、") || "无"}</p>
              </details>))}
          </Card>
        </Col>
      </Row>
      <Modal title={decision ? "确认批准并生成新基线" : "确认退回，保留当前生效基线"} open={decision !== undefined} okButtonProps={{ disabled: !canDecide }} onCancel={() => setDecision(undefined)} onOk={() => {
        ...
        <p>{input.title}</p>
        <p>审批意见：{opinion || "请先填写处理意见"}</p>
        <p>
          原版本 {base?.version}，申请成本影响{" "}
          {((proposed?.totalAmount ?? 0) - (base?.budgetAmount ?? 0)).toFixed(2)}
          万元。
        </p>
      </Modal>
    </>);
