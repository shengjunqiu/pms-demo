...
function ready(){const state=createBusinessState();const id=state.opportunities.at(-1)!.id;state.opportunities.at(-1)!.status='拟立项';const solution:SolutionVersion={id:'SV-1',version:1,submittedAt:'2026-09-09',submittedBy:'赵工',customerSituation:'数据分散',goals:'统一数据',scope:'20接口',boundaries:'无清洗',architecture:'微服务',implementation:'迭代',deliverables:'部署包',dependencies:'客户接口',assumptions:'接口开放',ownerId:'U-005',participants:[],startDate:'2026-09-09',endDate:'2026-09-20',attachments:['方案.pdf'],changeReason:''};const cost:CostVersion={id:'CV-1',solutionVersionId:'SV-1',version:1,total:113,submittedAt:'2026-09-09',submittedBy:'赵工',solutionFingerprint:'',feasibility:'可行',architecture:'微服务',reuse:'80%',customization:'20接口',environment:'容器',security:'鉴权',dependencies:'客户接口',risk:'交付窗口',attachments:[],lines:[{id:'L1',subjectId:'SUB-03',name:'服务设备',scope:'20接口',quantity:10,unit:'台',unitPrice:10,taxBasis:'不含税',taxRate:13,basis:'采购询价',risk:''}]};state.presales[id]={solutionVersions:[solution],costVersions:[cost],research:[],reviews:[{id:'R1',round:1,solutionVersionId:'SV-1',costVersionId:'CV-1',status:'通过',method:'专家会',plannedDate:'2026-09-09',experts:[],opinions:[],createdAt:'2026-09-09',createdBy:'李主任',corrections:[]}]};return {state:transition(state,{type:'estimate-create-draft',id,reviewId:'R1'},tech),id};}
describe('概算版本与冻结',()=>{
 it('来源双版本、税口径与合计一致，发布不直接冻结',()=>{const {state,id}=ready();const d=state.estimateDrafts[id];expect(estimateTotals(d).cost).toBe(113);const next=transition(state,{type:'estimate-publish',id},tech);const e=next.estimates.at(-1)!;expect(e.totalCost).toBe(113);expect(e.items.reduce((n,i)=>n+i.amount,0)).toBe(e.totalCost);expect(e.isFrozen).toBe(false);expect(next.estimateMeta[e.id].solutionVersionId).toBe('SV-1');expect(next.estimateMeta[e.id].costVersionId).toBe('CV-1');});
