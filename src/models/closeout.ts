import type { TemplateVersion } from "@/models/configuration";
export interface TemplateEvaluationScore {
  score: number;
  note: string;
  actorId: string;
  actor: string;
...
export interface PostEvaluation {
  ...
  templateScores?: Record<string, TemplateEvaluationScore>;
  templateScoreHistory?: ({ rowId: string } & TemplateEvaluationScore)[];
  id: string;
  projectId: string;
  status: "编制中" | "待确认" | "已完成";
  startedAt: string;
  ...
  snapshot: {
    ...
    changes: { id: string; title: string; status: string }[];
    requirements: number;
    bugs: number;
  };
  goals: Record<
    EvaluationGoal,
    { conclusion: "达成" | "部分达成" | "未达成" | ""; note: string }
  >;
  riskReview: string;
  changeReview: string;
  successes: string;
